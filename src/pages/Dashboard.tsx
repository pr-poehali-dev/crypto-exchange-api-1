...
interface CryptoWallet {
  network: string; address: string; memo?: string; label: string;
  symbol: string; min_deposit: number; created_at?: string;
}
interface CryptoDeposit {
  id: number; network: string; address: string; tx_hash?: string;
  amount?: number; currency: string; status: string;
  confirmations: number; auto: boolean; created_at: string;
}
...
const DEPOSIT_NETWORKS: Record<string, { label: string; symbol: string; color: string; icon: string; min: number; hint: string }> = {
  ...
  BNB:        { label: 'BNB BEP-20',  symbol: 'BNB',  color: 'text-yellow-400', icon: 'Coins',       min: 0.01,   hint: 'Сеть BNB Smart Chain' },
  TON:        { label: 'TON',          symbol: 'TON',  color: 'text-cyan-400',   icon: 'Diamond',     min: 1,      hint: 'Нативная сеть TON · обязателен MEMO' },
  SOL:        { label: 'Solana',       symbol: 'SOL',  color: 'text-purple-400', icon: 'Triangle',    min: 0.01,   hint: 'Нативная сеть Solana' },
};
...
const TX_LABELS: Record<string, { label: string; icon: string; color: string }> = {
  ...
  transfer_out:     { label: 'Перевод →',        icon: 'Send',            color: 'text-sell' },
  transfer_in:      { label: '← Перевод',        icon: 'Inbox',           color: 'text-buy' },
...
export default function Dashboard() {
  ...
  const [depDepositsLoaded, setDepDepositsLoaded] = useState(false);
  ...
  // Поллинг баланса пока открыт активный кошелёк
  useEffect(() => {
    if (activeWallet && tab === 'deposit') {
  ...
  const loadCryptoDeposits = async () => {
    const { ok, data } = await api.cryptoWallets.deposits();
    if (ok) { setCryptoDeposits((data as { deposits: CryptoDeposit[] }).deposits); setDepDepositsLoaded(true); }
  };
  const loadTxs = async () => {
  ...
  const generateWallet = async (network: string) => {
    ...
    const { ok, data } = await api.cryptoWallets.generate(network);
    setWalletLoading(false);
    if (ok) setActiveWallet(data as CryptoWallet);
  };
  ...
  const doSwap = async () => {
    ...
    if (ok) {
      ...
      setExAmount(''); setExQuote(null); refresh();
    } else {
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex gap-2 flex-wrap justify-end">
              ...
              <button onClick={() => setTab('deposit')} className="bg-primary text-background px-3 py-2 rounded-lg text-sm font-semibold flex items-center gap-1.5 hover:opacity-90">
        ...
        {tab === 'deposit' && (
          <div className="space-y-4">
            ...
            {!activeWallet && (
              <div className="bg-card border border-border rounded-xl p-6 space-y-5">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Выберите сеть — получите персональный адрес кошелька</p>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {Object.entries(DEPOSIT_NETWORKS).map(([net, info]) => (
                    <button key={net}
                      onClick={() => { setSelectedNetwork(net); generateWallet(net); }}
                      disabled={walletLoading && selectedNetwork === net}
                      className={`flex flex-col gap-2 p-4 rounded-xl border-2 text-left transition-all hover:border-primary/50 active:scale-95 ${selectedNetwork === net && walletLoading ? 'border-primary bg-primary/10 opacity-70' : 'border-border bg-secondary hover:bg-card'}`}>
                      <div className="flex items-center justify-between">
                        <Icon name={info.icon} size={22} className={info.color} />
                        {selectedNetwork === net && walletLoading && (
                          <Icon name="Loader" size={14} className="animate-spin text-muted-foreground" />
                        )}
                      </div>
                      <div>
                        <p className="font-semibold text-sm">{info.label}</p>
                        <p className="text-[11px] text-muted-foreground mt-0.5">мин. {info.min} {info.symbol}</p>
                      </div>
                    </button>
                  ))}
                </div>
                <div className="bg-yellow-400/10 border border-yellow-400/20 rounded-lg px-4 py-3 flex gap-2 text-sm">
                  <Icon name="AlertTriangle" size={15} className="text-yellow-400 shrink-0 mt-0.5" />
                  <span className="text-foreground/80">Отправляйте только указанный токен в выбранной сети. Переводы в другой сети будут потеряны.</span>
                </div>
              </div>
            )}
              ...
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-border flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <button onClick={() => { setActiveWallet(null); setSelectedNetwork(null); }}
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Персональный адрес пополнения</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-buy bg-buy/10 px-2.5 py-1.5 rounded-full">
                ...
                <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col items-center justify-center gap-4">
                    <div className="bg-white p-4 rounded-2xl shadow-lg">
                      <QRCodeSVG
                        value={activeWallet.memo ? `${activeWallet.address}?memo=${activeWallet.memo}` : activeWallet.address}
                        size={180}
                        level="M"
                      />
                    </div>
                    <p className="text-xs text-muted-foreground text-center">Отсканируйте QR или скопируйте адрес</p>
                  </div>

                  {/* Адрес и детали */}
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <p className="text-xs uppercase tracking-wider text-muted-foreground">Адрес кошелька</p>
                      <div className="flex items-center gap-2 bg-secondary rounded-xl px-4 py-3 border border-border">
                        <span className="flex-1 font-mono text-xs break-all leading-relaxed">{activeWallet.address}</span>
                        <button onClick={() => copy(activeWallet.address, 'addr')}
                          className="shrink-0 w-8 h-8 rounded-lg bg-card flex items-center justify-center hover:bg-muted transition-colors">
                          <Icon name={copied === 'addr' ? 'Check' : 'Copy'} size={14} className={copied === 'addr' ? 'text-buy' : 'text-muted-foreground'} />
                        </button>
                      </div>
                    </div>
                    ...
                    {activeWallet.memo && (
                      <div className="space-y-1.5">
                        <p className="text-xs uppercase tracking-wider text-muted-foreground">MEMO / TAG <span className="text-sell normal-case">(обязателен!)</span></p>
                        <div className="flex items-center gap-2 bg-sell/10 border border-sell/30 rounded-xl px-4 py-3">
                          <span className="flex-1 font-mono text-sm font-bold">{activeWallet.memo}</span>
                          <button onClick={() => copy(activeWallet.memo!, 'memo')}
                            className="shrink-0 w-8 h-8 rounded-lg bg-card flex items-center justify-center hover:bg-muted transition-colors">
                            <Icon name={copied === 'memo' ? 'Check' : 'Copy'} size={14} className={copied === 'memo' ? 'text-buy' : 'text-muted-foreground'} />
                          </button>
                        </div>
                        <p className="text-xs text-sell">Без MEMO средства будут утеряны!</p>
                      </div>
                    )}

                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="bg-secondary rounded-lg px-3 py-2.5">
                        <p className="text-[10px] text-muted-foreground uppercase">Минимум</p>
                        <p className="font-semibold mt-0.5">{activeWallet.min_deposit} {activeWallet.symbol}</p>
                      </div>
                      <div className="bg-secondary rounded-lg px-3 py-2.5">
                        <p className="text-[10px] text-muted-foreground uppercase">Зачисление</p>
                        <p className="font-semibold mt-0.5">Авто</p>
                      </div>
                    </div>

                    <div className="bg-primary/10 border border-primary/20 rounded-xl px-4 py-3 text-xs text-foreground/80 space-y-1">
                      <p className="flex items-center gap-1.5 font-medium text-primary"><Icon name="Info" size={13} /> Как это работает</p>
                      <p>Переведите крипту на этот адрес. Баланс обновится автоматически после подтверждения транзакции в блокчейне.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {cryptoDeposits.length > 0 && (
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                <div className="px-6 py-4 border-b border-border flex items-center justify-between">
                  <h3 className="font-semibold text-sm">История пополнений</h3>
                  <button onClick={loadCryptoDeposits} className="text-muted-foreground hover:text-foreground">
                    <Icon name="RefreshCw" size={14} />
                  </button>
                </div>
                <div className="divide-y divide-border">
                  {cryptoDeposits.slice(0, 10).map(dep => (
                    <div key={dep.id} className="flex items-center justify-between px-6 py-3">
                      <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full ${dep.status === 'confirmed' ? 'bg-buy' : 'bg-yellow-400 animate-pulse'}`} />
                        <div>
                          <p className="text-sm font-medium">{dep.network} · {dep.currency}</p>
                          <p className="text-xs text-muted-foreground">
                            {dep.tx_hash ? dep.tx_hash.slice(0, 16) + '...' : 'Ожидание'} · {new Date(dep.created_at).toLocaleDateString('ru')}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className={`font-mono-num text-sm font-semibold ${dep.status === 'confirmed' ? 'text-buy' : 'text-muted-foreground'}`}>
            ...
            {fiatInfo && !fiatOrder && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center bg-secondary border border-border rounded-lg px-4">
                    <input type="number" value={fiatAmount} onChange={e => setFiatAmount(e.target.value)}
                      placeholder="1000" className="flex-1 bg-transparent py-3 font-mono-num text-sm outline-none" />
                    <span className="text-muted-foreground text-sm">₽</span>
                  </div>
                  {fiatAmount && parseFloat(fiatAmount) >= 500 && (
                    <p className="text-xs text-muted-foreground">Получите ≈ <span className="text-foreground font-semibold">{((parseFloat(fiatAmount) * 0.98) / fiatInfo.rub_per_usdt).toFixed(2)} USDT</span></p>
                  )}
                </div>
                <button onClick={createFiatOrder} disabled={fiatLoading || !fiatAmount || parseFloat(fiatAmount) < 500}
                  ...
                  {fiatLoading ? 'Создаём заявку...' : 'Создать заявку'}
                </button>
              </>
            )}
            {fiatOrder && (
              <div className="space-y-4">
                <div className="bg-buy/10 border border-buy/30 rounded-xl p-4 text-sm space-y-1">
                  ...
                  <p className="text-muted-foreground mt-2">Переведите <strong className="text-foreground">{fiatOrder.amount_rub} ₽</strong> → получите <strong className="text-foreground">{fiatOrder.usdt_amount} USDT</strong></p>
                </div>
                {fiatInfo && (
                  <div className="space-y-2">
                    {[
                ...
                <p className="text-xs text-muted-foreground text-center">Обязательно укажите комментарий. Зачисление в течение 15 минут в рабочее время.</p>
          ...
          <div className="bg-card border border-border rounded-xl p-6 space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="relative flex items-center bg-secondary border border-border rounded-lg px-4">
                ...
                <input type="text" value={trTo}
                  onChange={e => { setTrTo(e.target.value); setTrChecked(null); checkUser(e.target.value); }}
                  placeholder="username" className="flex-1 bg-transparent py-3 text-sm outline-none" />
            ...
            <div className="space-y-2">
              ...
              {trAmount && <p className="text-xs text-muted-foreground">Комиссия: 0.1% ({(parseFloat(trAmount || '0') * 0.001).toFixed(6)} {trCurrency})</p>}
            </div>
            {trResult && (
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center bg-secondary border border-border rounded-lg px-4">
                <input type="number" value={exAmount}
                  onChange={e => { setExAmount(e.target.value); setExQuote(null); }}
                  placeholder="0.00" className="flex-1 bg-transparent py-3 font-mono-num text-sm outline-none" />
                <button onClick={() => { const b = String(getBalance(exFrom)); setExAmount(b); fetchQuote(exFrom, exTo, b); }}
                  className="text-xs text-primary ml-2">MAX</button>
              </div>
            </div>
            {exQuote && (
              <div className="bg-secondary rounded-xl p-4 space-y-2 text-sm">
                    ...
                    <div key={tx.id} className="flex items-center justify-between px-6 py-4">
                      ...
                      <div className="text-right">
                        <p className={`font-mono-num font-semibold text-sm ${info.color}`}>
                          {['transfer_out', 'exchange'].includes(tx.type) ? '-' : '+'}{tx.amount} {tx.currency}
                        </p>
